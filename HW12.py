    ...
    def days_to_birthday(self):
        if self.birthday == None:
            ...
            if  datetime(day=d_now.day, month=d_now.month, year=d_now.year) < datetime(day=self.birthday.value.day, month=self.birthday.value.month, year=d_now.year):
                diff = (datetime(day=self.birthday.value.day, month=self.birthday.value.month, year=d_now.year) - datetime(day=d_now.day, month=d_now.month, year=d_now.year)).days
                ...
                diff = (datetime(day=self.birthday.value.day, month=self.birthday.value.month, year=d_now.year+1) - datetime(day=d_now.day, month=d_now.month, year=d_now.year)).days
                return f'{diff} days to {self.name.value}`s birthday'
            elif datetime(day=d_now.day, month=d_now.month, year=d_now.year) == datetime(day=self.birthday.value.day, month=self.birthday.value.month, year=d_now.year):
...
def search_common(*args):
    ...
    for k, v in phone_book.data.items():
            if args[0].lower() in v.name.value.lower():
                searched_profiles.append(v.name.value)
            for i in v.phones:
                if args[0] in i.value:
                    searched_profiles.append(v.name.value)
